...
export const useTransactions = (limit = 50) => {
     ...
     */
    const fetch = useCallback(async (page = 0, bot, search) => {
        setIsLoading(true);
...
export const useLogs = () => {
     ...
     */
    const fetch = useCallback(async (bot, level, search, limit = 100) => {
        setIsLoading(true);
        const params = new URLSearchParams();
        params.append('limit', limit.toString());
        if (bot)
     ...
     */
    const startPolling = useCallback((bot, level, search) => {
        fetch(bot, level, search);
        pollTimerRef.current = setInterval(() => fetch(bot, level, search), 10000);
    }, [fetch]);
    /**
     * Stop auto-polling
     */
...
export const useApiCall = () => {
     ...
     */
    const call = useCallback(async (endpoint, method = 'POST', body) => {
        setIsLoading(true);
        const options = {
            method,
        };
